...
def update_v(
    critic: TrainState, value: TrainState, batch: Batch, expectile: float
) -> Tuple[TrainState, InfoDict]:
...
def update_q(
    critic: TrainState, target_value: TrainState, batch: Batch, discount: float
) -> Tuple[TrainState, InfoDict]:
...
def _update_jit(
    ...
    PRNGKey, TrainState, TrainState, TrainState, TrainState, TrainState, InfoDict
]:
...
class Learner(object):
    def __init__(
        ...
        target_critic_params = init_params(
            critic_def, [critic_key, observations, actions]
        )
