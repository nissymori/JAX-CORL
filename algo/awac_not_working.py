...
class AWACTrainer(NamedTuple):
    ...
    def update_critic(
        ...
        def critic_loss_fn(critic_params):
            ...
            next_actions = jnp.clip(dist.sample(seed=rng), -1.0, 1.0)
    ...
    def update_actor(
        agent, batch: Transition, rng: jax.random.PRNGKey, num_samples: int
    ) -> Tuple["AWACTrainer", Dict]:
        rng, value_rng, actor_rng = jax.random.split(rng, 3)
        dist = agent.actor.apply_fn(agent.actor.params, batch.observations)
        pi_actions = dist.sample(seed=value_rng)
        phi_q_1, phi_q_2 = agent.critic.apply_fn(agent.critic.params, batch.observations, pi_actions)
        v = jnp.minimum(phi_q_1, phi_q_2)

        def actor_loss_fn(actor_params):
            lim = 1 - 1e-5
            actions = jnp.clip(
                batch.actions, -lim, lim
            )  # clip actions to avoid log(0) in log_prob
            ...
            weights = jnp.minimum(
                jnp.exp(a / agent.config["beta"]), agent.config["exp_adv_max"]
            )
            weights = jax.lax.stop_gradient(weights)

            actor_loss = -(jax.nn.softmax(weights) * log_probs).sum()
            return actor_loss
    ...
    def update_n_times(
        ...
        for _ in range(n_updates):
            ...
            agent, actor_loss = agent.update_actor(batch, actor_rng, num_samples)
...
def create_trainer(
    ...
    actor_tx = optax.adamw(
        learning_rate=config.actor_lr, weight_decay=config.weight_decay
    )
    ...
    # initialize critic
    critic_def = ensemblize(Critic, num_qs=2)(config.critic_hidden_dims)
